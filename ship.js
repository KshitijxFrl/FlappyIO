...
export const spaceship = (function() {
  ...
  class _FlappyShipObject {
    constructor(scene) {
      this._scene = scene;
      this._sprite = scene.add.sprite(_Ship_POS_X, 100, 'bird');
      this._spriteTint = scene.add.sprite(_Ship_POS_X, 100, 'bird-colour');
      this._velocity = 0;
      this._dead = false;
    }
  ...
  class FlappyBird_Manual extends _FlappyShipObject {
    constructor(scene) {
      super(scene);
  ...
  class FlappyBird_NeuralNet extends _FlappyShipObject {
    constructor(config) {
      super(config.scene);

      this._model = new neuralNet.FFNeuralNetwork(config.pop_params.shapes);
    ...
    Update(params) {
      function _PipeParams(ship, pipe) {
        const distToPipe = (
            (pipe.X + pipe.Width) - ship.Bounds.left) / ship._config.config_width;
        const distToPipeB = (
            (pipe._sprite1.y - ship.Bounds.bottom) / ship._config.config_height) * 0.5 + 0.5;
        const distToPipeT = (
            (pipe._sprite2.y - ship.Bounds.top) / ship._config.config_height) * 0.5 + 0.5;
